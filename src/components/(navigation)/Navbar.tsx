      ...
      <div className="flex flex-col items-center z-50 fixed w-full">
        ...
        >
          <nav className="flex flex-col md:flex-row p-4 justify-center items-center md:justify-evenly md:w-[76%]">
            ...
            <button className="md:hidden text-[#1c1c1c]" onClick={() => setShowMenu(!showMenu)}>
            <XMarkIcon className="size-4 text-[#1c1c1c]" />
            </button>
          </nav>
        </div>
        <button onClick={() => setShowMenu(!showMenu)} className={`bg-gradient-to-r from-transparent via-coralRed to-transparent p-2 w-full flex items-center justify-center md:hidden ${showMenu ? "hidden" : ""}`}>
